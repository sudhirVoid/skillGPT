...
export class LibraryModalComponent {
  ...
  toastVisible = false;
  toastMessage: string =
    'Complete the topic at least 80% to Unlock the Flash cards and Quizzes';
  showToast() {
  ...
  navigateToFlashcard(topic: any) {
    ...
    const matchingFlashcard = this.flashcards.find(
      (flashCard: { bookId: any }) => flashCard.bookId === topic.book_id
    );
    ...
    this.dataShareService.currentData.subscribe((message) => {
      ...
      this.topics.map((topic: any) => {
        if (topic.percentOfBookRead >= 80) {
          ...
          this.topics.map((topic: any) => {
            if (!this.updatedTopics.some((updatedTopic: any) => updatedTopic.book_id === topic.book_id)) {
              this.updatedTopics.push(topic);
            }
          });
        }
      });
    ...
    return topics.map((topic) => {
      ...
      if (parseFloat(topic.percentOfBookRead) >= 80) {
        const matchingFlashcard = flashCards.find(
          (flashCard) => flashCard.bookId === topic.book_id
        );
        const flashCardCount = matchingFlashcard
          ? matchingFlashcard.flashCards.length
          : 0;
